...
WHERE prod_quantity <=30 and prod_quantity >= 12;

-- 8.	Display the id, the description, and the total value (product quantity * product price) of each product whose product type is HW.  
--Assign the column name TOTAL_VALUE to this calculation.

SELECT prod_id, prod_desc,prod_price*prod_quantity AS total_price from product 
WHERE prod_type = 'HW';
...
SELECT prod_id, prod_desc, prod_price *prod_quantity AS total_value FROM product
WHERE prod_type = 'HW';
...
SELECT COUNT(cust_balance) as total_customers from customer
...
SELECT prod_id, prod_desc, (prod_quantity * prod_price) AS prod_total_value FROM product
...
SELECT rep_id, SUM(cust_balance) AS rep_total_bal FROM customer
GROUP BY rep_id
ORDER BY rep_id;
...
WHERE prod_desc is NULL OR prod_desc = '';